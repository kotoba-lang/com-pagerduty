(ns pagerduty.main-test
  "Contract + behavioral test for the pagerduty-compat L5 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "pagerduty-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'pagerduty.main-test)]
