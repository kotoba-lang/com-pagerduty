(ns pagerduty.main
  "Kotodama WASM entrypoint for the Pagerduty clean-room actor (L5) — Clojure port.
  ...
  facts (`pagerduty.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "pagerduty")
(def tier "L5")
...
(def entity-specs
  [{:entity "Incident"         :plural "incidents"         :id-prefix "pagerdut_inc"
    :fields [:title :incidentNumber :status :urgency :incidentKey :assignedVia :resolvedAt] :required [:title :incidentNumber]
    :coerce {:incidentNumber :int}  :refs {}}
   {:entity "Service"          :plural "services"          :id-prefix "pagerdut_ser"
    :fields [:name :description :status :autoResolveTimeout :acknowledgementTimeout] :required [:name :description]
    :coerce {:autoResolveTimeout :int :acknowledgementTimeout :int} :refs {}}
   {:entity "EscalationPolicy" :plural "escalationpolicies" :id-prefix "pagerdut_esc"
    :fields [:name :description :numLoops :onCallHandoffNotifications] :required [:name :description]
    :coerce {:numLoops :int} :refs {}}
   {:entity "User"             :plural "users"             :id-prefix "pagerdut_use"
    :fields [:name :email :role :timeZone :jobTitle :invitationSent] :required [:name :email]
    :coerce {:invitationSent :bool} :refs {}}
   {:entity "Schedule"         :plural "schedules"         :id-prefix "pagerdut_sch"
    :fields [:name :description :timeZone] :required [:name :description]
    :coerce {} :refs {}}
   {:entity "Team"             :plural "teams"             :id-prefix "pagerdut_tea"
    :fields [:name :description :defaultRole] :required [:name :description]
    :coerce {} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"pagerduty.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "pagerduty-compat" :tier tier :entities entities} 200])
...
(defn start! [] :pagerduty-compat/ready)
